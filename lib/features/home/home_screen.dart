...
import '../../widgets/workshop_image.dart';
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return catalog.when(
      ...
      error: (_, _) => Scaffold(
        body: ErrorState(
          message: 'The local catalog failed to open. Restart the app. If it keeps failing, reinstall so the bundled database can copy again.',
  ...
  Widget build(BuildContext context) {
    if (vehicle == null) {
      return EmptyState(
        ...
        message: 'Pick a make, model, year and engine so results stay relevant. Hilux and 48V V-Active are first-class. Or start from a mate path below.',
        actionLabel: 'Open garage',
        onAction: () => context.push('/garage'),
        illustrationAsset: WorkshopArt.emptyGarage,
      );
    }
    return Card(
      child: ListTile(
        leading: const WorkshopImage(
          asset: WorkshopArt.vehicleActive,
          height: 48,
          width: 48,
          borderRadius: BorderRadius.all(Radius.circular(10)),
          semanticLabel: 'Active vehicle',
        ),
